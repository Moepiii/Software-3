...
## Compatibilidad con feature/gamificacion

- Se conservan los endpoints y las reglas existentes de experiencia, pagos,
  administración, reservas y navegación de personas y empresas.
- Los puntos por avance y la experiencia son métricas distintas: Mis Puntos
  utiliza el historial y los puntos base del curso; gamificación conserva sus
  100 EXP por curso finalizado y su cálculo de nivel cada 1000 EXP.
- Llegar al 100 % de avance clasifica el curso como completado en Mis Puntos,
  pero la inscripción sigue activa hasta que el administrador finaliza el curso.
  Así continúa siendo elegible para recibir la experiencia del flujo existente.
- Al finalizar el curso se acreditan los puntos restantes antes de continuar
  con la entrega de experiencia y el cambio de estado existentes. Repetir el
  mismo progreso o finalizar de nuevo un curso completado no duplica premios.
- Se rechaza la acreditación de avance de inscripciones canceladas.
- Aplicar `20260830010000_add_puntos` y regenerar el cliente Prisma antes de
  arrancar esta versión. No se modifica ninguna migración previa del equipo.
- Los cursos finalizados antes de aplicar esta migración conservan su experiencia;
  la migración no les acredita puntos retroactivos.

Validación adicional:

```sh
# Desde Backend, con una base aislada que ya tenga el esquema actualizado:
MIS_PUNTOS_TEST_DATABASE_URL='postgresql://usuario@localhost:puerto/pruebas' go test ./internal/repositories -run TestPuntosConviveConFinalizacionGamificacion -v
# Desde Frontend:
pnpm exec playwright test tests/mispuntos.spec.ts --project=chromium
```
